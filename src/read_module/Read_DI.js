module.exports = function (RED) {
  ...
  function YottaReadDiNode(config) {
    ...
    if(node.modbusQty>1) text = `...DI ${node.modelQty + node.modbusQty-1}`;
    node.status({fill:"grey",shape:"dot",text:"DI "+ node.modelQty + text}); //在外觀顯示該節點是read or write
  ...
  RED.nodes.registerType("Read_DI", YottaReadDiNode);

  RED.httpAdmin.get("/Read_DI/:command", RED.auth.needsPermission('commandfield.read'), function(req,res) {        
      res.json(moduleData);
  });
